...
def ask_ai_question(
    ...
    """
    logger.info("Rota /insights/ask acionada")
    try:
        # --- 1. COLETA DE DADOS RELEVANTES ---
        logger.debug("Coletando dados do sistema...")
        ...
        # Logs de verificação
        logger.debug(f"Encontrados {len(produtos_atuais_db)} produtos no estoque, {len(historico_top_vendas_db)} registros de top vendas, {len(movimentacoes_recentes_db)} movimentações recentes")


        # --- 2. FORMATAÇÃO DOS DADOS PARA A IA ---
        logger.debug("Formatando dados para o contexto da IA...")
        ...
        logger.debug(f"Contexto JSON final será enviado para a IA (tamanho: {len(system_data_json_string)} caracteres)")


        # --- 3. CHAMADA AO SERVIÇO DE IA ---
        logger.debug("Enviando contexto para o serviço de IA...")
        ...
        logger.error(f"Erro grave no endpoint /insights/ask: {e}", exc_info=True)
...
def produtos_estoque_critico(
    ...
    try:
        ...
        logger.error(f"Erro ao buscar produtos críticos: {e}", exc_info=True)
...
def produtos_alta_rotatividade(
    ...
    try:
        ...
        logger.error(f"Erro ao buscar produtos de alta rotatividade: {e}", exc_info=True)
...
def produtos_sem_movimento(
    ...
    try:
        ...
        logger.error(f"Erro ao buscar produtos sem movimento: {e}", exc_info=True)
...
def sugerir_compras(
    ...
    try:
        ...
        logger.error(f"Erro ao gerar sugestões de compra: {e}", exc_info=True)
