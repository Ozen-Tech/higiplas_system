...
"""
Módulo responsável pela criação do superusuário inicial do sistema.
...
def create_initial_superuser() -> None:
    ...
    2. Cria a empresa padrão se necessário
    3. Cria o superusuário com perfil ADMIN
    
    Raises:
        Exception: Se ocorrer erro durante a criação do superusuário
    """
    ...
    if not settings.SUPERUSER_EMAIL or not settings.SUPERUSER_PASSWORD:
        logger.warning(
            "Variáveis de ambiente do superusuário não definidas. Pulando criação."
        )
        return
    ...
    try:
        ...
        if user:
            logger.info(
                ...
                "já existe no banco de dados. Nenhuma ação necessária."
            )
            return

        logger.info(
            f"Superusuário não encontrado. Iniciando processo de criação "
            f"para '{settings.SUPERUSER_EMAIL}'."
        )
        ...
        empresa = (
            ...
            .filter(models.Empresa.nome == default_company_name)
            .first()
        )
        if not empresa:
